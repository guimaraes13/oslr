...
/* wwcmod: how can { f(W,Y) : hasWord(X,W) } do something for weighted features?
	 ...
	 2) { f(W,Y)=* : hasWord(X,W) } and compile to modified codes such that
	   a) doFeatureFindallDFS finds all outlinks WITH feature computation, so hasWord(x,W) returns a state with
	   feature { .. = z }
...
public class WamInterpreter {
    ...
    private static final Logger log = LogManager.getLogger(WamInterpreter.class);
    private static final int MAXDEPTH = 5;
    private static final int MAX_HASH = 1 << (Integer.SIZE - 2);
    private final List<FeatureBuilder> featureStack;
    private final WamProgram program;
    private final WamPlugin[] plugins;
    ...
    public ImmutableState saveState() {
        return this.state.immutableVersion();
    }
    ...
    public Map<Feature, Double> executeWithoutBranching() {
        return executeWithoutBranching(-1);
    }

    public Map<Feature, Double> executeWithoutBranching(boolean computeFeatures) {
        return executeWithoutBranching(-1, computeFeatures);
    }

    public Map<Feature, Double> executeWithoutBranching(int startAddress) {
        return executeWithoutBranching(startAddress, true);
    }
    ...
    public Map<Feature, Double> executeWithoutBranching(int startAddress, boolean computeFeatures) {
        ...
        while (!state.isFailed() && !state.isCompleted() && this.reportedFeatures.isEmpty() && state
                .getProgramCounter() >= 0) {
        ...
        try {
            switch (inst.opcode) {
                ...
                case pushfreevar:
                    ...
                    break;
                case pushboundvar:
                    pushboundvar(inst.i1);
                    break;
                case unifyconst:
                    unifyconst(inst.s, inst.i1);
                    break;
                case initfreevar:
                    initfreevar(inst.i1, inst.i2);
                    break;
                case unifyboundvar:
                    unifyboundvar(inst.i1, inst.i2);
                    break;
                case fclear:
                    fclear();
                    break;
                case fpushstart:
                    fpushstart(inst.s, inst.i1);
                    break;
                case fpushconst:
                    fpushconst(inst.s);
                    break;
                case fpushboundvar:
                    fpushboundvar(inst.i1);
                    break;
                case fpushweight:
                    fpushweight();
                    break;
    ...
    private void doFeatureFindallDFS(State state, int depth) throws LogicProgramException {
        ...
        if (depth >= MAXDEPTH) {
            throw new IllegalStateException("depth bound " + MAXDEPTH + " exceeded in feature computation");
        }
    ...
    public void unifyconst(String a, int relativeHeapIndex) throws LogicProgramException {
        ...
        if (state.hasConstantAt(ri)) {
            ...
            if (!state.hasFreeAt(ri)) {
                throw new LogicProgramException("ri(=" + ri + ") is neither free nor constant?");
            }
    ...
    public void unifyboundvar(int a, int relativeHeapIndex) throws LogicProgramException {
        ...
        if (ri == rj) {
            ...
            if (!state.hasFreeAt(ri) || !state.hasFreeAt(ri)) {
                throw new LogicProgramException("ri,rj not both free");
            }
            state.setHeap(rj, state.createVariableCell(ri)); // bind larger to smaller
            rj = ri;
        } else {
            if (!(ri > rj) || !state.hasFreeAt(ri) || !state.hasFreeAt(rj)) {
                throw new LogicProgramException("ri,rj not ordered and free");
            }
    ...
    public void fpushboundvar(int a) throws LogicProgramException {
        ...
        if (!state.hasConstantAt(ra)) {
            throw new LogicProgramException("variable in feature not bound to a constant");
        }
    ...
    public void freport() throws LogicProgramException {
        ...
        if (this.featureStack.isEmpty()) { reportFeature(new Feature("_no_features_"), 1.0); }
        this.state.incrementProgramCounter();
    }
    ...
    public void setArg(int k, int i, String value) throws LogicProgramException {
        int j = getHeapwiseIndex(k, i);
        int rj = state.dereference(j);
        if (!state.hasFreeAt(rj)) {
            throw new LogicProgramException("var " + rj + " is not free in setArg(" + k + "," + i + "," + value + ")" +
                                                    ":\n" + state);
        }
    ...
    public int canonicalHash(State root, State from) throws LogicProgramException {
        ...
        for (int k : root.getRegisters()) {
            ...
            if (from.hasConstantAt(j)) { hash = hash ^ from.getIdOfConstantAt(j) - 1; }
        }
    ...
    public String canonicalForm(State root, State from) throws LogicProgramException {
        ...
        for (int k : root.getRegisters()) {
            ...
            if (from.hasConstantAt(j)) {
                ...
                if (cid > this.getConstantTable().size()) {
                    throw new IllegalStateException("No value for constant id " + cid);
                }
