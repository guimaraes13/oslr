...
public class Compiler {
        ...
        for (Goal g : rule.features) {
            ...
            for (Argument a : g.getArgs()) {
                if (a.isConstant()) {
                    ...
                    throw new WAMSyntaxException("Unbound variable in feature in rule :" + rule);
                }
            }
        }
    }
    ...
    private void compileGoal(Goal g, WamProgram wamprog,
        ...
        wamprog.append(new Instruction(OP.callp, new StringBuilder(g.getFunctor()).append(WamInterpreter
                                                                                                  .JUMPTO_DELIMITER)
                .append(g.getArity()).toString()));
    }
